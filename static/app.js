/**
 * HONEYPOT SCAM DETECTION SYSTEM
...
(() => {
    ...
    const cursorCrosshair = $('#cursor-crosshair');
    ...
    function initCursorTracker() {
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            if (coordX) coordX.textContent = e.clientX;
            if (coordY) coordY.textContent = e.clientY;
        });
        ...
        document.addEventListener('mouseleave', () => {
            cursorCrosshair.classList.remove('active');
        });
    }
    ...
    function initSession() {
        ...
        chatStatus.textContent = 'ONLINE // AWAITING INPUT';
        ...
        fields.forEach(f => {
            const countEl = $(`#count-${f}`);
            const valEl = $(`#values-${f}`);
            ...
            if (valEl) {
                valEl.innerHTML = '<span class="empty-value">None extracted</span>';
            }
            if (fieldEl) {
                fieldEl.classList.remove('has-data');
            }
        });
        ...
        chatInput.placeholder = 'Enter scammer message...';
    ...
    function renderWelcome() {
        ...
        <div class="welcome-block">
            ...
            <h2 class="welcome-title">WELCOME TO THE HONEYPOT</h2>
            <p class="welcome-desc">You are role-playing as a <strong>SCAMMER</strong>. Send scam messages and observe the AI agent extract intelligence in real time.</p>

            <div class="instruction-cards">
                <div class="instruction-card">
                    <span class="instruction-num">[A]</span>
                    <span class="instruction-text">Use the <strong>SCRIPTS</strong> panel for pre-written 10-turn conversations</span>
                    <span class="corner-mark tl">+</span>
                    <span class="corner-mark br">+</span>
                </div>
                <div class="instruction-card highlight">
                    <span class="instruction-num">[B]</span>
                    <span class="instruction-text">Complete <strong>10 TURNS</strong> to unlock the full intelligence report</span>
                    <span class="corner-mark tl">+</span>
                    <span class="corner-mark br">+</span>
                </div>
            </div>
            ...
            <div class="quick-start-section">
                ...
                <div class="quick-start-buttons">
                    <button class="example-btn" data-message="URGENT: Your SBI account has been compromised. Your account will be blocked in 2 hours. Share your account number and OTP immediately to verify your identity.">
                        <span class="btn-inner">[01] BANK FRAUD</span>
                        <span class="corner-mark tl">+</span>
                        <span class="corner-mark br">+</span>
                    </button>
                    <button class="example-btn" data-message="URGENT: Your Aadhaar eKYC has expired. Your bank account will be BLOCKED in 24 hours. Call us immediately at +91-8876543210 to re-verify.">
                        <span class="btn-inner">[02] KYC FRAUD</span>
                        <span class="corner-mark tl">+</span>
                        <span class="corner-mark br">+</span>
                    </button>
                    <button class="example-btn" data-message="CONGRATULATIONS! You have won Rs 50,00,000 in the National Digital India Lottery 2024! Ticket number LT-2024-77654. Call now at +91-9988776655 to claim.">
                        <span class="btn-inner">[03] LOTTERY FRAUD</span>
                        <span class="corner-mark tl">+</span>
                        <span class="corner-mark br">+</span>
                    </button>
                    <button class="example-btn" data-message="URGENT LEGAL NOTICE: Income Tax Department. A tax evasion FIR ITD-2024-CR-55321 has been registered against your PAN. Call immediately: +91-9911223344.">
                        <span class="btn-inner">[04] TAX FRAUD</span>
                        <span class="corner-mark tl">+</span>
                        <span class="corner-mark br">+</span>
                    </button>
                </div>
    ...
    function buildScriptsPanel() {
        scriptsTabs.innerHTML = SCAMMER_SCRIPTS.map((s, i) =>
            `<button class="script-tab ${i === 0 ? 'active' : ''}" data-idx="${i}">
                <span class="script-tab-icon">[${s.icon}]</span>
                <span class="script-tab-label">${s.label}</span>
                <span class="script-tab-count">${s.turns.length}</span>
            </button>`
        ).join('');

        scriptsTabs.querySelectorAll('.script-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                activeScriptIdx = parseInt(btn.dataset.idx);
                scriptsTabs.querySelectorAll('.script-tab').forEach(b => b.classList.remove('active'));
    ...
    function renderScriptMessages() {
        ...
        scriptsMessages.innerHTML = script.turns.map((text, i) => {
            ...
            return `
            <div class="script-item ${isSent ? 'sent' : ''} ${isNext && !isSent ? 'active' : ''}"
                 data-turn="${i}" data-script="${activeScriptIdx}">
                <div>
                    <div class="turn-badge">${String(i + 1).padStart(2, '0')}</div>
                    <div class="sent-label">SENT</div>
                </div>
                <div class="script-text">${escapeHtml(text)}</div>
            </div>`;
        }).join('');

        scriptsMessages.querySelectorAll('.script-item:not(.sent)').forEach(item => {
    ...
    function loadTurnIntoInput(scriptIdx, turnIdx) {
        ...
        scriptsMessages.querySelectorAll('.script-item').forEach(el => {
    ...
    async function sendMessage(text) {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                addMessage('agent', 'SYSTEM ERROR // Please retry.');
            }
        } catch (err) {
            hideTyping();
            ...
            chatInput.focus();
            updateScriptPanel();
            if (!sessionComplete) chatStatus.textContent = 'ONLINE // AWAITING INPUT';
        }
    }
    ...
    async function viewResults() {
        ...
        resultsBody.innerHTML = '<div class="loading-text">LOADING INTELLIGENCE REPORT...</div>';
        ...
        try {
            ...
            if (data.status === 'success' && data.results) {
                ...
                resultsBody.innerHTML = `<div class="loading-text">${data.message}</div>`;
            } else {
                resultsBody.innerHTML = '<div class="loading-text">ERROR: Could not load results.</div>';
            }
        } catch {
            resultsBody.innerHTML = '<div class="loading-text">CONNECTION ERROR // Failed to load results.</div>';
        }
    }
    ...
    function renderResults(r) {
        ...
        const chips = (arr, cls = '') => {
            ...
            return `<div class="result-chips">${arr.map(v =>
                `<span class="value-chip ${cls}">${escapeHtml(v)}</span>`
            ).join('')}</div>`;
        };

        const scamStatusClass = r.scamDetected ? 'detected' : 'safe';
        const scamStatusText = r.scamDetected ? 'DETECTED' : 'NOT DETECTED';

        resultsBody.innerHTML = `
            <div class="results-section">
                <div class="section-title">
                    <span class="title-num">[R.1]</span>
                    <span>SESSION OVERVIEW</span>
                </div>
                <div class="results-grid">
                    <div class="result-item">
                        <div class="result-label">SESSION ID</div>
                        <div class="result-value highlight">${escapeHtml(r.sessionId || '---')}</div>
                    </div>
                    <div class="result-item">
                        <div class="result-label">SCAM STATUS</div>
                        <div class="result-value ${scamStatusClass}">${scamStatusText}</div>
                    </div>
                    <div class="result-item">
                        <div class="result-label">SCAM TYPE</div>
                        <div class="result-value">${escapeHtml(r.scamType || '---')}</div>
                    </div>
                    <div class="result-item">
                        <div class="result-label">CONFIDENCE</div>
                        <div class="result-value highlight">${r.confidenceLevel != null ? (r.confidenceLevel * 100).toFixed(0) + '%' : '---'}</div>
                    </div>
                    <div class="result-item">
                        <div class="result-label">MESSAGES EXCHANGED</div>
                        <div class="result-value highlight">${r.totalMessagesExchanged || 0}</div>
                    </div>
                    <div class="result-item">
                        <div class="result-label">ENGAGEMENT DURATION</div>
                        <div class="result-value highlight">${r.engagementDurationSeconds != null ? r.engagementDurationSeconds.toFixed(1) + 's' : '---'}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="results-section">
                <div class="section-title">
                    <span class="title-num">[R.2]</span>
                    <span>EXTRACTED INTELLIGENCE</span>
                </div>
                <div class="results-grid">
                    <div class="result-item">
                        <div class="result-label">PHONE NUMBERS</div>
                        ${chips(intel.phoneNumbers)}
                    </div>
                    <div class="result-item">
                        <div class="result-label">BANK ACCOUNTS</div>
                        ${chips(intel.bankAccounts)}
                    </div>
                    <div class="result-item">
                        <div class="result-label">UPI IDS</div>
                        ${chips(intel.upiIds)}
                    </div>
                    <div class="result-item">
                        <div class="result-label">EMAILS</div>
                        ${chips(intel.emailAddresses)}
                    </div>
                    <div class="result-item">
                        <div class="result-label">URLS</div>
                        ${chips(intel.phishingLinks || intel.urls)}
                    </div>
                    <div class="result-item">
                        <div class="result-label">IFSC CODES</div>
                        ${chips(intel.ifscCodes)}
                    </div>
                    <div class="result-item">
                        <div class="result-label">CASE IDS</div>
                        ${chips(intel.caseIds)}
                    </div>
                    <div class="result-item">
                        <div class="result-label">KEYWORDS</div>
                        ${chips(intel.suspiciousKeywords, 'keyword')}
                    </div>
                </div>
            </div>
    ...
    function bindExampleButtons() {
        document.querySelectorAll('.example-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                if (btn.dataset.message) sendMessage(btn.dataset.message);
            });
        });
    }
